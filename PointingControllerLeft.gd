...
func _physics_process(delta):
	...
	$LineRendererLeft.points[0] = origin + dir * 0.1
...
func _on_button_pressed(name):
	if name == "ax_button":
		if $LineRendererLeft.transparency == 1:
			$LineRendererLeft.transparency = 0
			visiblity = true
		else:
			visiblity = false 
			$LineRendererLeft.points[0] = Vector3(0,0,0)
			$LineRendererLeft.points[1] = Vector3(0,0,0)
			$LineRendererLeft.transparency = 1
